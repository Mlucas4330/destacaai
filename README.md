...
The LLM returns structured JSON that is passed directly to `CVTemplatePDF`, a [`@react-pdf/renderer`](https://react-pdf.org) component. The PDF is generated client-side via `pdf().toBlob()` — no backend, no remote resources, fully Manifest V3 compliant.
